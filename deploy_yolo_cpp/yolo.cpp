...
//反序列化引擎
ICudaEngine* init_model_yolo_engine(Parameters_yolo& cfg) {
...
//获得推理context格式
IExecutionContext* init_model_yolo_context(ICudaEngine& engine) {
...
//yolo在gpu上变量分配内存
void init_buffers(Parameters_yolo& cfg, ICudaEngine& engine,  float** input_data, float** output_data) {
    ...
    CHECK(cudaMallocHost((void**)input_data, cfg.batchSize * 3 * cfg.input_h * cfg.input_w * sizeof(float)));
    CHECK(cudaMallocHost((void**)output_data, cfg.batchSize * cfg.output_size * sizeof(float)));
    ...
    //// GPU分配内存空间
    //CHECK(cudaMalloc((void**)&gpu_input_buffer, cfg.batchSize * 3 * cfg.input_h * cfg.input_w * sizeof(float)));
    //CHECK(cudaMalloc((void**)&gpu_output_buffer, cfg.batchSize * cfg.output_size * sizeof(float)));
...
void doInference(Parameters_yolo& cfg, IExecutionContext& context, float* input, float* output)
{
    ...
    cudaMalloc(&buffers[0], cfg.batchSize * 3 * cfg.input_h * cfg.input_w * sizeof(float));
    cudaMalloc(&buffers[1], cfg.batchSize * cfg.output_size * sizeof(float));
    // Create stream
    cudaStream_t stream;
    CHECK(cudaStreamCreate(&stream));
    // DMA input batch data to device, infer on the batch asynchronously, and DMA output back to host
    CHECK(cudaMemcpyAsync(buffers[0], input, cfg.batchSize * 3 * cfg.input_h * cfg.input_w * sizeof(float), cudaMemcpyHostToDevice, stream));
    context.enqueue(cfg.batchSize, buffers, stream, nullptr); //推理
    CHECK(cudaMemcpyAsync(output, buffers[1], cfg.batchSize * cfg.output_size * sizeof(float), cudaMemcpyDeviceToHost, stream)); // 将gpu的buffers值赋值给host
...
void preprocess_data(Parameters_yolo& cfg, float* input_data, cv::Mat img) {
...
void infer_img_demo(Parameters_yolo& cfg) {
    ...
    //定义变量申明
    float* input_data = nullptr;  // host数据输入变量
    float* output_data = nullptr; //host数据输出变量申明



    ICudaEngine* engine = init_model_yolo_engine(cfg);
    IExecutionContext* context = init_model_yolo_context(*engine);

    init_buffers(cfg, *engine, &input_data, &output_data);
    ...
    for (int i = 0; i < cycle_num; i++) {
        ...
        preprocess_data(cfg, input_data, img);
        ...
        img = draw_rect_yolo(img, results);
...
void infer_video_demo(Parameters_yolo& cfg) {
    ...
    if (!capture.isOpened()) {      printf("could not read this video file...\n");    }
    ...
    while (capture.read(frame)) {
        ...
        preprocess_data(cfg, input_data, frame);
